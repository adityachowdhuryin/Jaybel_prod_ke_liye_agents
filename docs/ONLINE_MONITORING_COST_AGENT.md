...
**Cloud Run Job mode (default after deploy):** the job runs Cloud Trace **list without a server-side `+online_evaluator` filter** (`--scan-without-list-filter`), then **post-filters to traces whose spans mention your online evaluator** (same needles as the scan-mode matcher). This avoids ingesting every `cost_metrics_agent` chat turn: only runs that carry evaluator binding in exported spans are written (closer to “online monitor” traces). **Optional widen:** set `ONLINE_EVAL_SCAN_GEN_AI_AGENT_NAME=cost_metrics_agent` *and* only if needed `ONLINE_EVAL_SYNC_INCLUDE_AGENT_TRACES_WITHOUT_EVAL_LABELS=1` to restore the broader “all agent turns” behavior (what produced 10 rows from 10 seed prompts). Cap on Trace list examination: `ONLINE_EVAL_SYNC_SCAN_MAX_LIST_TRACES` (default 3000); cap on writes: `ONLINE_EVAL_SYNC_MAX_TRACES` (default 200).

**Metrics vs Console:** many traces still have **`metrics: {}` in Firestore** because rubric scores / explanations are often **not present on Cloud Trace span labels** (you confirmed this on a known trace). Older documents with filled `metrics` usually came from **`--metrics-overrides`** (Console Evaluation tab) or rare export luck. For new evaluated runs, keep using overrides or accept empty `metrics` until Google exports per-trace rubrics.
